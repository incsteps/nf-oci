...
The plugin supports several authentication methods. By default (`authType = 'auto'`)
it picks the most appropriate one for the current environment, preferring
credential-less methods so the same configuration works locally and inside
Kubernetes without shipping API keys into your containers.

You can force a specific method with the `oci.authType` config option (or the
...
| `auto` (default)
| Detect the best method: inline credentials, then OKE Workload Identity, then
  Resource Principals, then the local `~/.oci/config` file.
...
| `instance_principal`
| *Instance Principals* — for processes running on any OCI compute instance,
  including OKE worker nodes. The instance's dynamic group grants access.

| `resource_principal`
| *Resource Principals* — for OCI Functions and similar managed resources.
...
The service account token is read from the standard pod mount path
(`/var/run/secrets/kubernetes.io/serviceaccount/token`). Override it with
`oci.tokenPath` if your cluster mounts it elsewhere.

With `authType = 'auto'` (the default) the plugin detects an OKE Workload Identity
environment automatically, so explicitly setting `authType` is optional.
